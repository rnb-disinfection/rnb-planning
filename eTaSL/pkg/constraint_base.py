...
def make_point_pair_constraint(obj1, obj2, varname, constraint_name, make_error=True, point1=None, point2=None):
    error_statement = ""
    if make_error:
        error_val = "error_target = error_target + abs(dist_{varname}))".format(varname=varname)
...
def make_dir_constraint(pointer1, pointer2, name, constraint_name, make_error=True):
    error_statement = ""
    if make_error:
        error_val = "error_target = error_target + abs(angle_{name})".format(name=name)
        error_statement = error_val+'\n ctx:setOutputExpression("error",error_target)'
    return """
    vec1 = vector{vec1}
    vec2 = vector{vec2}
...
def make_orientation_constraint(framer1, framer2, name, constraint_name, make_error=True):
    error_statement = ""
    if make_error:
        error_val = "error_target = error_target + abs(orientation_{name})".format(name=name)
        error_statement = error_val+'\n ctx:setOutputExpression("error",error_target)'
    ...
    """.format(
        T1=framer1.object.get_transformation(),vec11=vec11, vec12=vec12,
        T2=framer2.object.get_transformation(),vec21=vec21, vec22=vec22,
        name=name) + \
    """
    Constraint{{
        context=ctx,
...
def make_directed_point_constraint(pointer1, pointer2, name, make_error=True, point1=None, point2=None):
    ...
    if make_error:
        error_val = "error_target = error_target + abs(dist_{constraint_name_point})+abs(angle_{constraint_name_dir})".format(
...
def make_oriented_point_constraint(framer1, framer2, name, make_error=True, point1=None, point2=None):
    ...
    if make_error:
        error_val = "error_target = error_target + abs(dist_{constraint_name_point})+abs(orientation_{constraint_name_ori})".format(
...
def make_joint_constraints(joint_names, make_error=True):
    ...
    if make_error:
        error_val = "error_target = error_target + "+error_statement[:-1]
        error_statement = error_val+'\n ctx:setOutputExpression("error",error_target)'
