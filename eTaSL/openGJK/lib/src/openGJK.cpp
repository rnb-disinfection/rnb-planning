...
double csFunction( int nCoordsA, double *inCoordsA, int nCoordsB, double *inCoordsB )
{
    ...
    for (j = 0; j < bd1.numpoints; j++)
        bd1.coord.push_back(Point3(inCoordsA[ 0*bd1.numpoints + j],
                                   inCoordsA[ 1*bd1.numpoints + j],
                                   inCoordsA[ 2*bd1.numpoints + j]));

    bd2.coord.clear();
    for (j = 0; j < bd2.numpoints; j++)
        bd2.coord.push_back(Point3(inCoordsB[ 0*bd2.numpoints + j],
                                   inCoordsB[ 1*bd2.numpoints + j],
                                   inCoordsB[ 2*bd2.numpoints + j]));
