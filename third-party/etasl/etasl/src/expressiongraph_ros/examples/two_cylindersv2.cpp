...
void desired_joint_positions(Context::Ptr ctx, const std::string& name, const std::map<std::string,double>& jval, double K, double weight) {
    for ( std::map<std::string,double>::const_iterator it = jval.begin(); it!= jval.end(); ++it) {
        ...
        CHECK( addConstraint(ctx,name+":"+it->first, e, K, weight,2) == 0 ); 
    }
}
...
cout<<"size of r: "<<r.size()<<endl;
    ...
    Expression<Vector>::Ptr p = p2-p1;

    Expression<double>::Ptr f1_ref = Constant(0.15)*sin(ctx->time*Constant(5.0*M_PI/4.0));
          ctx->addScalarVariable("along cyl. axis 1","feature",0.1, Constant(1.0));

    CHECK( addConstraint(ctx,"perpendicular L with distance vec", dot(p,unit_z(rotation(cylL))), 10.0, 1.0, 0) ==0 );
    CHECK( addConstraint(ctx,"perpendicular R with distance vec", dot(p,unit_z(rotation(cylR))), 10.0, 1.0, 0) ==0 );
    CHECK( addConstraint(ctx,"perpendicular L with R", dot(unit_z(rotation(cylL)),unit_z(rotation(cylR))), 10.0, 1.0, 2) ==0 );
    CHECK( addConstraint(ctx,"distance", norm(p) - Constant(radiusL+radiusR), 10.0, 1.0, 2)  ==0 );
    CHECK( addConstraint(ctx,"f1 position",
                       f1-f1_ref,
                       10.0, 0.01, 2) ==0);

    // with a low weight constraint : keep the cylinders together:
    Expression<Vector>::Ptr v = origin(cylR)-origin(cylL);
    //CHECK( ctx->addConstraint("two_cylinder_together_x", coord_x(v), 10.0, 0.01, 2) ==0 );
    //CHECK( ctx->addConstraint("two_cylinder_together_y", coord_y(v), 10.0, 0.01, 2) ==0 );
    //CHECK( ctx->addConstraint("two_cylinder_together_z", coord_z(v), 10.0, 0.01, 2) ==0 );
    CHECK( addInequalityConstraint(ctx,"ine_f1", f1,-0.05, 10.0,0.1, 10.0, 1, 1) ==0 )
    CHECK( addInequalityConstraint(ctx,"ine_f2", f2,-0.1, 10.0,0.05, 10.0, 1, 1) ==0 )
    ...
    CHECK(addConstraint(ctx,"left_shoulder_joint", 
                       left_shoulder_joint+Constant(0.1)*sin(ctx->time*Constant(1.0*M_PI/3.0))
                       - Constant(0.5), 
                       10.0, 0.01, 2) ==0 );
    ...
    double const TOTAL_TIME = 30;
    ctx->addMonitor( "fixed_duration",  ctx->time ,-1.0 ,TOTAL_TIME, "exit","");
    ...
    // ======================================================================================== 
    ros::init(argc,argv,"two_cylindersv2");
    ros::NodeHandle nh;
    ...
    std::ofstream f("data.m");//it happens that the file is written in $HOME/.ros/data.m if lauchfile is used
    ctx->addOutput<double>("time", "matlab",ctx->time);
    ctx->addOutput<double>("f1","matlab",f1);
    ctx->addOutput<double>("f2","matlab",f2);
