...
int main(int argc, char* argv[]) {
   ...
   Context::Ptr ctx = create_context();
    ...
    ctx->addType("feature");
        //-----------------------------------------------------    
        // define an INPUT variable that can be used in the loop:
        //-----------------------------------------------------    
        // make the variable dependant on time:
        // This is done by constructing a int std::vector and filling it with the variables you want to depend on.
        // since our variable could change at each time step, we make it dependant on the time variable, to ensure
        // that it is updated each time the time changes.  These variables will also determine the how the (possibly non-zero part of the ) Jacobian
        // looks for this variable:
    std::vector<int> ndx;
    int time_ndx = ctx->getScalarNdx("time");
    ndx.push_back( time_ndx );
        // construct variable :
    VariableType<double>::Ptr my_input = Variable<double>(ndx);
    //  or you can also write: VariableType<double>::Ptr my_input = Variable(time_ndx,1);
        // give the variable a value:
    my_input->setValue(0.0);
    ...
    // using the input variable my_input in the following constraint :
    CHECK( addConstraint(ctx,"distance", norm(p2-p1) - Constant(radiusL+radiusR) - my_input, 10.0, 1.0, 2)  ==0 );
    ...
    do {
        c++;
        if (c> 1000) {
            // put some signal in the input variable, coming from somewhere else:
            my_input->setValue(0.15+0.04*sin(c*0.01*0.2));
        }
        int retval = solver.updateStep(sample_time);
        ctx->checkMonitors();
        if (retval!=0) {
