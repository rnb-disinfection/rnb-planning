...
public:
     ...
     *
     */
    InputType(int _variable_number, double _defaultvalue, int _derivation_number=-1, double _DT=0.0):
        FunctionType<double>("input"),
        variable_number(_variable_number),
        val(_defaultvalue),
        derivation_number(_derivation_number),
        DT(_DT) {
    ...
    virtual void getDependencies(std::set<int>& varset) {
        ...
        if (derivation_number>=0){
            varset.insert( derivation_number);
        }
    }
    virtual void getScalarDependencies(std::set<int>& varset) {
        varset.insert( variable_number);
    ...
    virtual double derivative(int i) {
        if (variable_number == i) {
            return 1.0;
        } else if ((derivation_number== i)&&(DT != 0.0))  {
            return -(1.0 / DT);
        } else {
            return 0.0;
        }
    ...
    virtual Expression<ResultType>::Ptr clone() {
        Expression<ResultType>::Ptr expr(
            new InputType( variable_number, val, derivation_number, DT)
        );
        return expr;
    }
...
* and whose derivative with number derivative_number is equal to 1.
*/
inline Expression<double>::Ptr derivation(int variable_number, double default_value, int derivation_number, double dt  ) {
    Expression<double>::Ptr var(
            new InputType( variable_number, default_value, derivation_number, dt)
    );
    return var;
}

inline Expression<double>::Ptr derivation(int variable_number, int derivation_number, double dt) {
    Expression<double>::Ptr var(
            new InputType( variable_number, 0.0, derivation_number, dt )
    );
    return var;
}
