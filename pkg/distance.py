...
def distance_pt_pt(pt1, pt2, dist1, dist2): # (N_sim, N_col, 1, 3) / (N_sim, N_col, 1)
    dist_ = tf.linalg.norm(pt2-pt1, axis=-1)
...
def distance_pt_ln(pt1, ln2, dist1, dist2): # (N_sim, N_col, 1, 3), (N_sim, N_col, 2, 3) / (N_sim, N_col, 1)
    dpts = ln2-pt1
    dpt_nm = tf.linalg.norm(dpts,axis=-1)
    ...
    abo_v, abo_nm = tf.linalg.normalize(abo, axis=-1)
...
def distance_pt_pl(pt1, pl2, dist1, dist2, N_sim, N_col): # (N_sim, N_col, 1, 3), (N_sim, N_col, 4, 3) / (N_sim, N_col, 1)
    ...
    dist_, flag_ = distance_pt_ln(tf.repeat(pt1, 4, axis=-3), ln2, 0, 0)
    dist_ = K.min(tf.reshape(dist_, (N_sim, N_col, 4)), axis=-1, keepdims=True)


    po = pt1-pl2
    pl_sides = pl2_perm-pl2
    pl2_perp = tf.linalg.cross(tf.gather(pl_sides, [0], axis=-2), tf.gather(pl_sides, [1], axis=-2))
    pl2_perp, _ = tf.linalg.normalize(pl2_perp, axis=-1)
...
def distance_mesh(mesh1, mesh2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6):
    ...
    dist_, flag = test_collision_batch(mesh1, mesh2, v_batch, flag_default, dist_default, IterationAllowed=IterationAllowed)
    dist = dist_ - (dist1+dist2)
    flag = tf.less_equal(dist, 0)
    return dist, flag

@tf.function
def distance_mesh_2_8(mesh1, mesh2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6):
    v_batch = K.mean(mesh1, axis=-2, keepdims=True)-K.mean(mesh2, axis=-2, keepdims=True)
    v_batch = tf.linalg.cross(v_batch, x_batch)+tf.linalg.cross(v_batch, y_batch)
    dist_, flag = test_collision_batch(mesh1, mesh2, v_batch, flag_default, dist_default, IterationAllowed=IterationAllowed)
    dist = dist_ - (dist1+dist2)
...
@tf.function
def distance_mesh_4_4(mesh1, mesh2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6):
    v_batch = K.mean(mesh1, axis=-2, keepdims=True)-K.mean(mesh2, axis=-2, keepdims=True)
    v_batch = tf.linalg.cross(v_batch, x_batch)+tf.linalg.cross(v_batch, y_batch)
    dist_, flag = test_collision_batch(mesh1, mesh2, v_batch, flag_default, dist_default, IterationAllowed=IterationAllowed)
    dist = dist_ - (dist1+dist2)
...
@tf.function
def distance_mesh_4_8(mesh1, mesh2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6):
    v_batch = K.mean(mesh1, axis=-2, keepdims=True)-K.mean(mesh2, axis=-2, keepdims=True)
    v_batch = tf.linalg.cross(v_batch, x_batch)+tf.linalg.cross(v_batch, y_batch)
    dist_, flag = test_collision_batch(mesh1, mesh2, v_batch, flag_default, dist_default, IterationAllowed=IterationAllowed)
    dist = dist_ - (dist1+dist2)
...
@tf.function
def distance_mesh_8_8(mesh1, mesh2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6):
    v_batch = K.mean(mesh1, axis=-2, keepdims=True)-K.mean(mesh2, axis=-2, keepdims=True)
    v_batch = tf.linalg.cross(v_batch, x_batch)+tf.linalg.cross(v_batch, y_batch)
    dist_, flag = test_collision_batch(mesh1, mesh2, v_batch, flag_default, dist_default, IterationAllowed=IterationAllowed)
    dist = dist_ - (dist1+dist2)
...
#     return dist, flag
    ...
    dist_, flag_ = distance_pt_ln(tf.repeat(pt1, 12, axis=-3), ln2, 0, 0)
    dist_ = K.min(tf.reshape(dist_, (N_sim, N_col, 12)), axis=-1, keepdims=True)

    fc2_perm1 = tf.gather(bx2, tf.constant([0,1,2,3, 4,5,6,7, 0,1,5,4, 1,2,6,5, 2,3,7,6, 3,0,4,7]), axis=-2)
    fc2_perm2 = tf.gather(bx2, tf.constant([1,2,3,0, 5,6,7,4, 1,5,4,0, 2,6,5,1, 3,7,6,2, 0,4,7,3]), axis=-2)
    po = pt1-fc2_perm1
    ...
    fc2_perp, _ = tf.linalg.normalize(fc2_perp, axis=-1)
    ...
    inbox = tf.cast(inbox, tf.float32)

    dist = oncorner*dist_ + noncorner*K.min((1-inside)*1000+dist__, axis=-2) - K.sum(K.min(dist__,axis=-2, keepdims=True)*inbox, axis=-1)
...
def distance_ln_ln(ln1, ln2, dist1, dist2, N_sim, N_col, zeros_pt): # (N_sim, N_col, 1, 3), (N_sim, N_col, 4, 3) / (N_sim, N_col, 1)
    ...
    dist, flag = distance_pt_pl(zeros_pt, pl2, 0, 0, N_sim, N_col)
...
def distance_ln_pl(ln1, pl2, dist1, dist2, N_sim, N_col, zeros_pt): # (N_sim, N_col, 1, 3), (N_sim, N_col, 4, 3) / (N_sim, N_col, 1)
    ...
    dist, flag = distance_pt_bx(zeros_pt, bx2, dist1, dist2, N_sim, N_col)
    return dist, flag
...
#     dist_, flag_ = distance_pt_bx(pt1, bx2_, 0, 0, N_sim, N_col*2)
#     dist_ = K.min(tf.reshape(dist_, (N_sim, N_col, 2)), axis=-1, keepdims=True)
...
#     dist__, flag__ = distance_ln_ln(ln1_, ln2_, 0, 0, N_sim, N_col*12, zeros_pt_)
#     dist__ = K.min(tf.reshape(dist__, (N_sim, N_col, 12)), axis=-1, keepdims=True)
#     flag__ = tf.reduce_any(tf.reshape(flag__, (N_sim, N_col, 12)), axis=-1, keepdims=True)
#     dist = K.min([dist_, dist__], axis=0) - (dist1+dist2)
#     flag = tf.less_equal(dist, 0)
    return dist, flag
...
def distance_pl_pl(pl1, pl2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6): # (N_sim, N_col, 1, 3), (N_sim, N_col, 4, 3) / (N_sim, N_col, 1)
    dist, flag = distance_mesh_4_4(pl1, pl2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=IterationAllowed)
    return dist, flag
...
def distance_pl_bx(pl1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6): # (N_sim, N_col, 1, 3), (N_sim, N_col, 4, 3) / (N_sim, N_col, 1)
    dist, flag = distance_mesh_4_8(pl1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=IterationAllowed)
    return dist, flag
...
def distance_bx_bx(bx1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=6): # (N_sim, N_col, 1, 3), (N_sim, N_col, 4, 3) / (N_sim, N_col, 1)
    dist, flag = distance_mesh_8_8(bx1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch, IterationAllowed=IterationAllowed)
    return dist, flag

@tf.function
def test_all(pt1, pt2, ln1, ln2, pl1, pl2, bx1, bx2, dist1, dist2, zeros_pt, N_sim, N_col, flag_default, dist_default, x_batch, y_batch):
    dist1, flag = distance_pt_pt(pt1, pt2, dist1, dist2)
    dist2, flag = distance_pt_ln(pt1, ln2, dist1, dist2)
    dist3, flag = distance_pt_pl(pt1, pl2, dist1, dist2, N_sim, N_col)
    dist4, flag = distance_pt_bx(pt1, bx2, dist1, dist2, N_sim, N_col)
    dist5, flag = distance_ln_ln(ln1, ln2, dist1, dist2, N_sim, N_col, zeros_pt)
    dist6, flag = distance_ln_pl(ln1, pl2, dist1, dist2, N_sim, N_col, zeros_pt)
    dist7, flag = distance_ln_bx(ln1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch)
    dist8, flag = distance_pl_pl(pl1, pl2, dist1, dist2, flag_default, dist_default, x_batch, y_batch)
    dist9, flag = distance_pl_bx(pl1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch)
    dist10, flag = distance_bx_bx(bx1, bx2, dist1, dist2, flag_default, dist_default, x_batch, y_batch)
