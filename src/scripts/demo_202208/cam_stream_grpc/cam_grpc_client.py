...
from .demo_utils.heuristics import *
from .demo_utils.rotation_utils import *
from .demo_utils.utils import TextColors
...
# @param  intrins      Camera intrinsic parameters
# @param  depth_scale  multiplier for depthmap
...
class RemoteCam_Client:
    def __init__(self, HOST_IP="localhost", PORT=10509, MAX_MESSAGE_LENGTH=10000000):
        ...
        print("Initialize Client Class to receive color, depth image from remote camera server")
    ...
    ##
    # @brief  set merge option of masks for one object
    ...
    ##
    # @brief   ratio to make point cloud for computation time (avoid to generate pcd from whole data)
    def set_pcd_ratio(self, ratio=0.3):
        self.ratio = ratio


    ##
    # @brief   level to judge whether detection result can be used or not
    ...
    def inference(self, sd, color_img):
        ...
        mask_out_list_res = sd.inference(color_img=img_res)
    ...
    # @param   Tc          transfrom from robot base to camera
    # @param   name_mask   object name to detect
    ...
    def detect(self, sd, Tc=None, name_mask=None, visualize=False):
        ...
        mask_out_list = self.inference(sd, color_img=cdp.color)
        ...
        for name, micp in sorted(hrule_targets_dict.items()):
            ...
            if micp.make_pcd(cdp, ratio=self.ratio):
                ...
                if hrule.parent in detect_dict:
                    ...
                    if hrule.parent in self.initial_list.keys():
